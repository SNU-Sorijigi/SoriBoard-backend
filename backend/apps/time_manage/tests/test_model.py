...
class UserTests(APITestCase):
    def test_create_user(self):
        ...
        data = {'name': 'John Doe', 'major': 'Computer Science', 'year_id': '20', 'is_ob': False}
    ...
    def test_update_user(self):
        user = User.objects.create(name='John Doe', major='Computer Science', year_id='20', is_ob=False)
        url = reverse('user-detail', args=[user.id])
        data = {'name': 'Jane Doe', 'major': 'Electrical Engineering', 'year_id': '21', 'is_ob': True}
...
class SemesterTests(APITestCase):
    def test_create_semester(self):
        ...
        data = {'year': 2021, 'semester_num': 1, 'total_time': 4, 'start_time': '09:00:00', 'end_time': '17:00:00', 'rest_time': 10}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_update_semester(self):
        semester = Semester.objects.create(year=2021, semester_num=1, total_time=4, start_time='09:00:00', end_time='17:00:00', rest_time=10)
        url = reverse('semester-detail', args=[semester.id])
        data = {'year': 2022, 'semester_num': 2, 'total_time': 5, 'start_time': '08:00:00', 'end_time': '18:00:00', 'rest_time': 15}
        response = self.client.put(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
